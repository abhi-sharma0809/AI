...
def playGame(quickMove, token, show=False):
  ...
  while True:
    ...
    # Determine the move
    if tknToPlay != token: mv = random.choice([move for move in moves])     # Random's turn
    ...
    brd = makeMove(brd, findMoves(brd, tknToPlay).get(mv), tknToPlay) 
    brd = "".join(ch for ch in brd.values())
#Only place in miniMod that I altered, as my makeMove takes seperate arguements
    #my findMoves return a dictionary of moves as keys and 'traces' as values- each trace is a list of indexes that need to be flipped
    #makeMoves takes in the board, the traceList, and the tkn
...
for arg in args:
  if len(arg)==1 and arg in "xXoO": tkn = arg.upper()
