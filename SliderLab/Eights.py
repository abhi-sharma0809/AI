...
def findLevelDict(puzzle):
    ...
    while(parseMe):
       print(levelDict)
       nextItm = parseMe.pop(0)
       for neighbor in neighbors(nextItm):
           if neighbor not in dctSeen:
                layer = len(solve(puzzle,neighbor))-1
                levelDict[neighbor]=layer
                parseMe.append(neighbor)
                dctSeen[neighbor] = nextItm
    return levelDict
