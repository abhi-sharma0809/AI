...
def run():
    if args:
        ...
        steps = [0, 0]
        pz = ['1', '2','3', '4', '5', '6', '7', '8', '_']
        goal = ['1', '2','3', '4', '5', '6', '7', '8', '_']
        dimension(pz)
        startTime = time.process_time()
        for i in range(PATHSTOTEST):
            random.shuffle(pz)
            random.shuffle(goal)
            path = solve("".join(pz), "".join(goal))
            if path:
                steps[0] += 1
                steps[1] += (len(path)-1)
        print(f"Time: {float('%.3g' % (time.process_time()-startTime))}s")
        print(f"Puzzles Tested: {PATHSTOTEST}")
        print(f"Solvable puzzles: {steps[0]}")
        print(f"Average path length: {steps[1]/steps[0]}")
